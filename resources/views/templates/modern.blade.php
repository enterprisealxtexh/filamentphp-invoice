...
<head>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        body { font-family: 'DejaVu Sans', Arial, sans-serif; font-size: 12px; line-height: 1.6; color: #2d3748; background: #fff; }
        .invoice-container { max-width: 800px; margin: 0 auto; }
        .header-bar { background-color: #667eea; padding: 40px; color: #fff; }
        .header-content { display: table; width: 100%; }
        .header-left { display: table-cell; width: 60%; vertical-align: middle; }
        .header-right { display: table-cell; width: 40%; vertical-align: middle; text-align: right; }
        .company-logo { max-width: 150px; max-height: 60px; margin-bottom: 15px; }
        .company-name { font-size: 28px; font-weight: bold; margin-bottom: 5px; }
        .invoice-label { font-size: 14px; opacity: 0.8; text-transform: uppercase; letter-spacing: 2px; }
        .invoice-number { font-size: 24px; font-weight: bold; }
        .content-area { padding: 40px; }
        .info-row { display: table; width: 100%; margin-bottom: 40px; }
        .info-col { display: table-cell; width: 33.33%; vertical-align: top; }
        .info-box { background: #f7fafc; padding: 20px; border-radius: 8px; margin-right: 15px; }
        .info-col:last-child .info-box { margin-right: 0; }
        .info-label { font-size: 10px; text-transform: uppercase; letter-spacing: 1px; color: #718096; margin-bottom: 8px; }
        ...
        .status-cancelled { background: #fefcbf; color: #744210; }
        .items-table { width: 100%; border-collapse: collapse; margin-bottom: 30px; }
        .items-table th { background: #667eea; color: #fff; padding: 15px 12px; text-align: left; font-size: 11px; text-transform: uppercase; letter-spacing: 1px; }
        ...
        .item-description { font-size: 11px; color: #718096; margin-top: 4px; }
        .summary-section { display: table; width: 100%; }
        .summary-left { display: table-cell; width: 55%; vertical-align: top; padding-right: 30px; }
        .summary-right { display: table-cell; width: 45%; vertical-align: top; }
        .notes-box { background: #f7fafc; padding: 20px; border-radius: 8px; border-left: 4px solid #667eea; }
        .notes-title { font-weight: bold; color: #2d3748; margin-bottom: 10px; }
        .totals-box { background: #f7fafc; padding: 20px; border-radius: 8px; }
        .total-row { display: table; width: 100%; padding: 8px 0; }
        .total-label { display: table-cell; color: #718096; }
        .total-value { display: table-cell; text-align: right; font-weight: 600; color: #2d3748; }
        .grand-total { border-top: 2px solid #667eea; margin-top: 10px; padding-top: 15px; }
        .grand-total .total-label, .grand-total .total-value { font-size: 16px; color: #667eea; font-weight: bold; }
            ...
            <div class="info-row">
                ...
                <div class="info-col">
                    <div class="info-box">
                        ...
                        <div class="info-value">{{ $invoice->currency ?? $settings->default_currency }}</div>
                    </div>
                </div>
            </div>
                ...
                <div class="summary-right">
                    <div class="totals-box">
                        ...
                        <div class="total-row grand-total">
                            ...
                            <div class="total-value">{{ number_format($invoice->total, 2) }} {{ $invoice->currency ?? $settings->default_currency }}</div>
                        </div>
                        @if($invoice->paid > 0)
