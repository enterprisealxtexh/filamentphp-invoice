...
<head>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        body { font-family: 'DejaVu Sans', Arial, sans-serif; font-size: 12px; line-height: 1.5; color: #1e293b; background: #fff; }
        .invoice-container { max-width: 800px; margin: 0 auto; }
        .header { background: #0f172a; color: #fff; padding: 30px 40px; display: table; width: 100%; }
        ...
        .company-logo { max-width: 160px; max-height: 50px; margin-bottom: 10px; }
        .company-name { font-size: 22px; font-weight: bold; }
        .invoice-badge { background: #3b82f6; display: inline-block; padding: 8px 24px; font-size: 12px; font-weight: bold; text-transform: uppercase; letter-spacing: 2px; }
        .sub-header { background: #f1f5f9; padding: 20px 40px; display: table; width: 100%; }
        .sub-header-item { display: table-cell; width: 25%; }
        .sub-label { font-size: 10px; text-transform: uppercase; color: #64748b; margin-bottom: 4px; }
        .sub-value { font-size: 13px; font-weight: 600; color: #0f172a; }
        .content { padding: 40px; }
        .parties-section { display: table; width: 100%; margin-bottom: 40px; }
        .party-col { display: table-cell; width: 50%; vertical-align: top; }
        .party-box { background: #f8fafc; border-left: 4px solid #3b82f6; padding: 20px; margin-right: 20px; }
        .party-col:last-child .party-box { margin-right: 0; border-left-color: #10b981; }
        .party-label { font-size: 10px; text-transform: uppercase; letter-spacing: 1px; color: #64748b; margin-bottom: 10px; }
        .party-name { font-size: 15px; font-weight: bold; color: #0f172a; margin-bottom: 8px; }
        .party-details { font-size: 12px; color: #475569; line-height: 1.7; }
        .status-badge { display: inline-block; padding: 5px 14px; border-radius: 4px; font-size: 11px; font-weight: bold; text-transform: uppercase; }
        ...
        .items-table th { background: #0f172a; color: #fff; padding: 14px 12px; text-align: left; font-size: 11px; text-transform: uppercase; letter-spacing: 0.5px; }
        .items-table td { padding: 16px 12px; border-bottom: 1px solid #e2e8f0; }
        .items-table tbody tr:nth-child(even) { background: #f8fafc; }
        .text-right { text-align: right; }
        .text-center { text-align: center; }
        .item-name { font-weight: 600; color: #0f172a; }
        .item-description { font-size: 11px; color: #64748b; margin-top: 4px; }
        ...
        .notes-box { background: #fffbeb; border: 1px solid #fef3c7; padding: 20px; border-radius: 6px; }
        .notes-title { font-weight: bold; color: #92400e; margin-bottom: 10px; font-size: 11px; text-transform: uppercase; }
        .notes-content { color: #78350f; font-size: 12px; }
        .totals-box { background: #0f172a; color: #fff; padding: 20px; border-radius: 6px; }
        .totals-row { display: table; width: 100%; padding: 8px 0; border-bottom: 1px solid #334155; }
        .totals-row:last-child { border-bottom: none; }
        .totals-label { display: table-cell; color: #94a3b8; font-size: 12px; }
        .totals-value { display: table-cell; text-align: right; font-weight: 600; }
        .grand-total { background: #3b82f6; margin: 15px -20px -20px; padding: 15px 20px; border-radius: 0 0 6px 6px; }
        .grand-total .totals-label, .grand-total .totals-value { font-size: 14px; font-weight: bold; color: #fff; }
                ...
                <div class="bottom-right">
                    <div class="totals-box">
                        ...
                        <div class="totals-row grand-total">
                            ...
                            <div class="totals-value">{{ number_format($invoice->total, 2) }} {{ $invoice->currency ?? $settings->default_currency }}</div>
                        </div>
                    </div>
                </div>
            </div>
