...
<head>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        body { font-family: 'DejaVu Sans', Arial, sans-serif; font-size: 12px; line-height: 1.5; color: #333; background: #fff; }
        .invoice-container { max-width: 800px; margin: 0 auto; padding: 40px; }
        .invoice-header { display: table; width: 100%; margin-bottom: 40px; border-bottom: 2px solid #333; padding-bottom: 20px; }
        .invoice-header-left { display: table-cell; width: 50%; vertical-align: top; }
        .invoice-header-right { display: table-cell; width: 50%; vertical-align: top; text-align: right; }
        ...
        .invoice-meta-left, .invoice-meta-right { display: table-cell; width: 50%; vertical-align: top; }
        .meta-section { margin-bottom: 20px; }
        .meta-label { font-size: 10px; text-transform: uppercase; color: #999; margin-bottom: 5px; }
        ...
        .items-table th { background: #f8f9fa; border-bottom: 2px solid #333; padding: 12px 8px; text-align: left; font-size: 11px; text-transform: uppercase; color: #666; }
        .items-table td { padding: 12px 8px; border-bottom: 1px solid #e9ecef; }
        .items-table .text-right { text-align: right; }
        .items-table .text-center { text-align: center; }
        .item-description { font-size: 10px; color: #666; margin-top: 4px; }
        .totals-section { display: table; width: 100%; margin-bottom: 30px; }
        .totals-left { display: table-cell; width: 60%; }
        ...
        .totals-table .grand-total { font-size: 16px; border-top: 2px solid #333; }
        .notes-section { background: #f8f9fa; padding: 20px; border-radius: 4px; margin-bottom: 20px; }
        .notes-title { font-weight: bold; margin-bottom: 10px; color: #333; }
            ...
            <div class="totals-right">
                <table class="totals-table">
                    <tr>
                        ...
                        <td class="value">{{ number_format($invoice->total, 2) }} {{ $invoice->currency ?? $settings->default_currency }}</td>
                    </tr>
                    @if($invoice->vat > 0)
                        <tr>
                            <td class="label">Tax</td>
                            <td class="value">{{ number_format($invoice->vat, 2) }} {{ $invoice->currency ?? $settings->default_currency }}</td>
                        </tr>
                    @endif
                    @if($invoice->discount > 0)
                        <tr>
                            <td class="label">Discount</td>
                            <td class="value">-{{ number_format($invoice->discount, 2) }} {{ $invoice->currency ?? $settings->default_currency }}</td>
                        </tr>
                    @endif
                    @if($invoice->shipping > 0)
                        <tr>
                            <td class="label">Shipping</td>
                            <td class="value">{{ number_format($invoice->shipping, 2) }} {{ $invoice->currency ?? $settings->default_currency }}</td>
                        </tr>
                    @endif
                    ...
                    @if($invoice->paid > 0)
                        <tr>
                            ...
                            <td class="value">-{{ number_format($invoice->paid, 2) }} {{ $invoice->currency ?? $settings->default_currency }}</td>
                        </tr>
                        <tr>
                            <td class="label">Balance Due</td>
                            <td class="value">{{ number_format($invoice->total - $invoice->paid, 2) }} {{ $invoice->currency ?? $settings->default_currency }}</td>
                        </tr>
                    @endif
                </table>
            </div>
